...
## Signal research governance (Phase 2.2A)

The [Signal Research Framework](SIGNAL_RESEARCH_FRAMEWORK.md) defines documentation/workflow metadata for research identity, lifecycle status, input and availability semantics, methodology identity, artifact provenance, validation, and phase-gate decisions. These concepts do not add database columns or impose a universal evidence JSON structure. `available` and `insufficient_history` are shared component-status meanings; a methodology may define reasoned `unavailable`, while structural failure is never a normal status. `evidence_quality` continues to mean input completeness/availability/trustworthiness rather than forecast confidence.

## Signal research artifact files and validation (Phases 2.2B–2.2D)

The frozen [Signal Research Artifact Standard](SIGNAL_RESEARCH_ARTIFACT_STANDARD.md) defines file-level `research_artifacts_v1`. `manifest.json` owns research/run identity, provenance, configuration, execution, and inventory; `selected_observations.json` owns the exact selected input set; row-level CSVs own their declared evidence grain; `results.json` owns machine-readable conclusions and the research decision; `validation.json` owns independent-check results; `report.md` owns human narrative; and `README.md` owns reproduction instructions. `artifact_standard_version`, `research_version`, `methodology_id`, and outer `signal_version` are distinct identities. Artifact raw-file hashes are SHA-256 of exact bytes, while `selected_input_snapshot_hash` is a canonical identity-projection hash. Phase 2.2C validates these mechanics through `validate-research-artifacts`; they remain research-file contracts, not market-data or signal-observation database fields.

## Treasury 2s10s research (Phase 2.2D; not production)

Research `spread_bp = 100 × (DGS10_percent − DGS2_percent)` uses exact same-date selected inputs, no forward fill or unmatched-date fallback. Native legs remain percent; the existing Phase 1 spread remains percentage points. Research bp conversion does not change that stored indicator. Positive/zero/inverted are direct descriptive sign labels, not approved production categorical states. Net/path evidence is in bp, exploratory slope in bp per aligned observation, and prior-only empirical strict/weak bands in percent with explicit tie counts. Missing publication timestamps remain unknown; current-vintage observations do not establish event-time availability. See [contract](../notebooks/treasury_curve/research_contract.json) and [results](../notebooks/treasury_curve/outputs/results.json).
